define(['knockout', 'jquery', 'hammer', 'gesturehandler', 'animationmanager', 'utils', 'gesture', 'pauseinfo', 'jumpinfo', 'skipinfo'],
  function (ko, $, Hammer, gestureHandler, animationManager, Utils, Gesture, PauseInfo, JumpInfo, SkipInfo) {
    ...
    function reduceInstallBtn() {
      $("#infoButton").animate({height: '7%'}, 
      {
       duration: 1000,
       complete: function(){
         setTimeout(function() {growInstallBtn();},2000) ;
      }
    });
    }

    function growInstallBtn() {
      console.log("grow");
      $("#infoButton").animate({height: '9%'}, 
      {
       duration: 700,
       complete: function(){
           reduceInstallBtn()
...
function openEndScreen(){
      ...
      function canPlay() {
        ...
        if (video.get(0).currentTime * 1000 > 0) {
          ...
          if (gestureHandler.currentGestureIndex() === 0 && gestureHandler.isVideoStarted() === false) {
            ...
            if (self.opSystem === self.OP_IOS) {
              webkit.messageHandlers.observe.postMessage("video_did_start_playing");
            }            
          }; 
        } else {
      ...
      video.get(0).onended = function () {
        if (self.opSystem === self.OP_IOS) {
          webkit.messageHandlers.observe.postMessage("video_did_end_playing");
        }
