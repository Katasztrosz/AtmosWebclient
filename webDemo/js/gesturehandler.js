...
define('gesturehandler', ['knockout', 'jquery', 'hammer', 'utils', 'gestureinfo', 'tiltdetector', 'gesturetype'],
  ...
  var gestureHandler = function () {
    ...
    self.holdEnd = ko.observable(),
      ...
      self.isTimeoutSet = ko.observable(false),
...
/*
      video.on("seeked", function (e) {
        if (self.gestureInfo() !== undefined) {
          // var curtime = Math.round(e.target.currentTime * 1000);
          // console.log("OnSeeked EVENT, current time: " + curtime);
          clearTimeout(self.timers()[self.currentTimerIndex()]);
          self.handleGestureWithEvent();
        }
      });
*/
      ...
      hammertime.get('press').set({ threshold: 1000 });


  //Can use the handlers by uncommenting the lines
     //self.enableHammer();
    };
...
//         self.setupHammer();
//         // self.videOnSeeked();
//         self.initHammerGestureRecoginizers();
//       };
...
//       self.setupHammer = function () {
//         hammertime = new Hammer(video.get(0));
//         hammertime.get('swipe').set({ direction: Hammer.DIRECTION_ALL });
//         hammertime.get('swipe').set({ evenType: Hammer.EVENT_START });
//         hammertime.get('swipe').set({ threshold: 1 });
//         hammertime.get('swipe').set({ velocity: 0.1 });
...
//     self.initHammerGestureRecoginizers = function () {
//       //Can use the handlers by uncommenting the lines
...
//       hammertime.on('pan swipe', swipeHandler);
//       hammertime.on('pinchin', pinchinHandler);
//       hammertime.on('pinchout', pinchoutHandler);
//       };

// >>>>>>> refs/remotes/origin/Second_Adam-AtmosBranch
    //To Enable gestures.
    ...
    self.disableHammer = function () {
      ...
      console.log("gestures are disabled");
    };

      // self.hideMenuWrapperIfNeeded = function () {
      //   if (self.isVideoStarted() && !self.isVideoCompleted()) {
      //     $("#menuWrapper").stop().fadeTo(500, 0.0, function () {
      //       self.showInfoMenu();
      //       self.initMenu();
      //     });
      //   }
      ...
      function tapHandler(ev) {
        ...
        if (self.gestureInfo()) {
          if (self.currentGestureIndex() < self.gestures().length && self.isVideoStarted()  && isEventValid) {
            ...
            console.log('tap detected enable' + video.get(0).currentTime);
      ...
      function pressupHandler(ev) {
        ...
        //   for (var j = 0; j < self.gestures()[self.currentGestureIndex()].length; j++) {
        //     var gesture = self.gestures()[self.currentGestureIndex()][j];
        //     if (gesture.type() == GestureType.HOLD && self.isVideoPlaying()) {
        //       if (video.get(0).currentTime * 1000 > self.holdEnd()) {
        //         self.handleGestureWithEvent(ev, gesture);        //*Not tested
        //       } else {
        //         video.get(0).pause();
      ...
      self.isSwipeDirectionOk = function (ev, direction) {
        ...
        switch (ev.direction) {
          case 2:
            ...
            return (self.orientation == 1 && direction == 270) || (self.orientation == 0 && direction == 180);
          case 4:
            console.log('rightswipe');
            return (self.orientation == 1 && direction == 90) || (self.orientation == 0 && direction == 0);
          case 8:
            console.log('upswipe');
            return (((self.orientation == 1 && direction == 0) || (self.orientation == 0 && direction == 270))); 
          case 16:
            console.log('downswipe');
            return (self.orientation == 1 && direction == 180) || (self.orientation == 0 && direction == 90);
          default:
            return false;
        }
      };  
    ...
    function handleSomeJumpEvent(ev, gesture) {
      ...
      if (gesture !== null) {
        ...
        if (currentPosition >= gesture.attack() - Utils.GESTURE_LOADING_TIME) {
            ...
            //if seeked than onSeeked ovent fires
            // clearTimeout(self.timers()[self.currentTimerIndex()]);
            // self.handleGesture();
        }else{
      ...
      //   for (var i = 0; i < video.get(0).seekable.length; i++) {
      //     if (video.get(0).currentTime > video.get(0).seekable.start(i) && video.get(0).currentTime < video.get(0).seekable.end(i)) {
      //       return true
      //     }
      //   }
      //   return false
...
//Uses the handleGesture() in every situation, so i call that inmediately
        ...
        if (gesture && self.gestureInfo().gesture()) {
          ...
          //After the final gesture the video plays till the end, than we can open the Store with a storeClick
            ...
            self.gestureInfo().index(null);
              ...
              // self.preloadGestureImage(self.gestures()[index]);
              // var timeout = self.gestures()[index][0].attack() < Utils.GESTURE_LOADING_TIME ? 0 : self.gestures()[index][0].attack() - Utils.GESTURE_LOADING_TIME;
              // // console.log("attack time: " + self.gestures()[index][0].attack());
              // // console.log("cure time: " + video.get(0).currentTime * 1000);
              // timeout -= video.get(0).currentTime * 1000;
              // if (timeout < 0) timeout = 0;
              // console.log("set timeout: " + timeout + ' for index: ' + index);
              // self.setTimerForIndex(index, timeout);
...
//       self.videOnSeeked = function () {
//         video.on("seeked", function (e) {
//           if (self.gestureInfo()) {
//             // console.log("current time after seek: " + video.get(0).currentTime * 1000);
//             var index = 0;
//             for (var i = 0; i < self.gestures().length; i++) {
//               if (self.gestures()[i][0].attack() < video.get(0).currentTime * 1000) {
//                 index++;
//               }
//             }
...
//             }
//             index--;
//             self.gestureInfo().index(index);
//             self.handleGestureWithEvent(null, gestureToSeek);
//             isSeekInProgress = false;
//           }
//         });
//       };
    ...
    self.timeoutFunc = function (index, minAttack) {
        console.log("timeoutFunc()" + video.get(0).currentTime);
        ...
        console.log("minattack " + minAttack);
        console.log("curpos " + curpos);
        // var timeout = self.gestures()[index].timeout() + Utils.GESTURE_LOADING_TIME;
        if (delay > 0) {
          setTimeout(function () {
            // setTimeout(function () {
            applyGesture(index);
      ...
      function applyGesture(index) {
        self.gestures()[index][0].target()
        console.log("ApplyGestures()")
        console.log("enable attack " + self.gestures()[index][0].attack());
        self.enableHammer();      //Enables gestures
        /*if (self.isTimeoutFinished() && self.gestures()[index].type() !== "pause" && self.gestures()[index].type() !== "jump"
          && self.gestures()[index].type() !== "jumptap" && self.gestures()[index].type() !== "jumpswipe" && self.gestures()[index].type() !== "jumpdoubletap") {
          video.get(0).pause();
          self.isPaused(true);
          self.isVideoPlaying(false);
        } else */
        //if (self.gestures()[index].type() === "jumptap" || self.gestures()[index].type() === "jumpswipe" || self.gestures()[index].type() === "jumpdoubletap") {
          var curpos = Math.round(video.get(0).currentTime * 1000);
          var timeout = self.gestures()[index][0].target() - curpos;
 ...
 /*   GESTURE HANDLERBŐL 
       ...
       if (gestureHandler.gestureInfo().gesture().type() === "lefttilt" || gestureHandler.gestureInfo().gesture().type() === "righttilt") {
          addTiltDetectorEventListener();
          tiltEnd = gestureHandler.gestureInfo().gesture().attack() + gestureHandler.gestureInfo().gesture().timer();
        } else if (gestureHandler.gestureInfo().gesture().type() === "hold") {
          gestureHandler.holdEnd(gestureHandler.gestureInfo().gesture().attack() + gestureHandler.gestureInfo().gesture().timer());
        } else if (gestureHandler.gestureInfo().gesture().type() === "keeptapping") {
          gestureHandler.keeptappingEnd(gestureHandler.gestureInfo().gesture().attack() + gestureHandler.gestureInfo().gesture().timer());
          gestureHandler.keepTappingAttack(gestureHandler.gestureInfo().gesture().attack());
        }

        if (gestureHandler.gestureInfo().gesture().show() === "yes") {
          animationManager.showAnimation(gestureHandler.gestureInfo().gesture());
        }
*/
      ...
      self.setTimerForIndex = function (index) {
        ...
        var minAttack = self.gestures()[index][0].attack();
        var i = 0;
        // while(self.gestures()[index][i+1]){
        //   if(self.gestures()[index][i+1].attack() < minAttack){
        //     minAttack = self.gestures()[index][i+1].attack();
        //     i++;
        //   }
        // }
        ...
        if(self.gestures()[index][0].type() == GestureType.HOLD){
          ...
          self.gestureInfo(new GestureInfo(index, self.gestures()[index]));  
            ...
            setTimeout(function(){ 
              ...
              console.log("video is paused bcuse of HOLD " + video.get(0).currentTime*1000); 
              console.log(" hold attack: " + self.gestures()[index][0].attack()); 
            }
              , timeout);
          }else{
              if(!self.isVideoStarted){      //For the first gesture
                  self.timers().push(setTimeout(self.timeoutFunc, timeout, self.currentGestureIndex()));
